...
const Cart = () => {
    ...
    const updateQuantity = async (productId, newQuantity) => {
        ...
        const response = await eventApi.checkQuantity(productId);
        if(response.quantity < newQuantity){
            notification["error"]({
                message: `Notification`,
                description:
                    'Sản phẩm không đủ số lượng! vui lòng chọn lại! Sản phẩm còn lại:' + response.quantity,
            });
            return;
        }
        console.log(response);
